...
try:
    ...
    # Setup logging
    logging.basicConfig(
        filename=_LOG_DIR / f"saraphina_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s'
    )

    # Log system information
    logging.info("Starting Saraphina...")
    ...
    logging.info(f"Memory Usage: {memory_info.percent}% used of {memory_info.total / (1024 ** 2):.2f} MB")
